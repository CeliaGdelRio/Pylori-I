...
classdef HRRTree < rigidBodyTree
    ...
    methods (Access = public)
        %% CONSTRUCTOR
        function obj = HRRTree(NLinks, Assembly)
        ...
        %% SECTION IKINE
        function jointPositions = sectionIKine(obj, index, coords, type, BetaMaxLink)
            %Inicializaciones   
            ...
            [BetaMaxS] = obj.betaMax(index, BetaMaxLink);
            %fprintf('BetaS %d\n', beta);
        ...
        function [newConfig, e, iterTot] = iKine(obj, tform, varargin)
            ...
            %Ajuste a solución de Mínima Norma
%             while (e > ef)&&(iter < iterMax)
...
%             end
            ...
            iter = 0;
            %Ajuste Cinemático
            while (e > ef)&&(iter < iterMax)&&(iterTot < iterTotMax)
        ...
        function [newConfig, e, iter] = iKineGradientDescent(obj, tform, varargin)
            ...
            maxIter = 250; %Máximo de iteraciones.
            
            ef = 0.1;   %Error deseado
            k = 1;      %Constante de correcion
            
            m = 250;             %Iteración de convergecia
            epsilon = 10 * ef;  %Valor de convergencia
            
            BetaMinLink = -0.23;      %Ángulo mínimo de un disco
            BetaMaxLink = 0.23;       %Ángulo máximo de un disco (13.1964º)
           ...
           %CÁLCULO DE BETA Y PHI:
               ...
               for i = 1:obj.NSections                   
                   ...
                   %de_Beta:
                       %Cálculo de la nueva configuracion para las articulaciones:
                           ...
                           %pequeño incremento en BETA
                           jointPos = obj.sectionIKine(i, [beta(i)+0.0001 phi(i) 1], 'hrr', BetaMaxLink);
                   ...
                   %de_Phi:
                       %Cálculo de la nueva configuracion para las articulaciones:
                           ...
                           %Cinemática inversa de las secciones metiendo un
                           %pequeño incremento en PHI
                           jointPos = obj.sectionIKine(i, [beta(i) phi(i)+0.0001 1], 'hrr', BetaMaxLink);
               ...
               %phi = phi - de_Phi * k * e;
               beta = beta - de_Beta * k * log(e);
               phi = phi - de_Phi * k * log(e);
           ...
           %CÁLCULO DE NUEVA POSICIÓN
               ...
               for i = 1:obj.NSections                   
                   ...
                   %Cálculo de la nueva configuracion para las articulaciones:
                       jointPos = obj.sectionIKine(i, [beta(i) phi(i) 1], 'hrr', BetaMaxLink);
                       sectionConfig = joints2config(i, jointPos);                   
                       newConfig(firstLink:lastLink) = sectionConfig(1:obj.NLinks(i));
               end
        ...
        function [newConfig, e, iter] = iKineHRR(obj, tform, varargin)
            BetaMinLink = -0.239;      %Ángulo mínimo de un disco
            BetaMaxLink = 0.239;       %Ángulo máximo de un disco (13.71º)
            ...
            for i = 1:obj.NSections
                    sectionJoints = obj.sectionIKine(i, [q(i) q(i+obj.NSections) 1], 'hrr', BetaMaxLink);
                    for j = 1:length(sectionJoints)
                        if sectionJoints(j) > BetaMaxLink
                            sectionJoints(j) = BetaMaxLink;
                        elseif sectionJoints(j) < BetaMinLink
                            sectionJoints(j) = BetaMinLink;
                        end
                    end
        ...
        function [J] = jacobianHRR(obj, BetaMaxLink, varargin)
            %Inicializaciones:
            delta = 1e-6;                %Diferencial de la variable
            config = obj.Config;        %Configuración de partida
            jacobianType ='Analytic';   %Tipo de jacobiana deseada            
...
%             movie(M,50,5)
        ...
        function [n,colision] = colisiones (obj, newConfig)
            ...
            for j=1:obj.TotalLinks-2
                for i=j:obj.TotalLinks-2
                    disco1.Pose = getTransform (obj, newConfig, obj.BodyNames{1,j});
                    disco2.Pose = getTransform (obj, newConfig, obj.BodyNames{1,i+2});

                    if checkCollision(disco1,disco2)==1
                        n=n+1;
                        colision (n,:) ={obj.BodyNames{1,j},obj.BodyNames{1,i+2}};
                    end     
                end
            end
            %fprintf('\nSe han producido %d colisiones\n', n);

        end
        
        %% CALCULA BETA MAX        
        function [BetaMaxS] = betaMax(obj, index, BetaMaxLink)
            BetaMaxS = 0;
            alpha = -obj.Assembly{index}(1);
            [BetaMaxLink, phi] = corrigeBetaPhi(BetaMaxLink, 0);
            for i = 1:obj.NLinks(index)
                alpha = alpha + obj.Assembly{index}(i);
                BetaMaxS = BetaMaxS + BetaMaxLink * (cos(alpha - phi));
            end
            %fprintf('BetaMaxLink %d\nBetaMaxS %d\n',BetaMaxLink,BetaMaxS);
        end
    end
    ...
    methods (Access = private)
        ...
        function createSections(obj)
            for i = 1:obj.NSections
                ...
                newSection = HRRSection(i, obj.NLinks(i), obj.Assembly{i});
